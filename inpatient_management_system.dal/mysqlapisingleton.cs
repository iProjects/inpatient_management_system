...
namespace inpatient_management_system.dal
{
    ...
    public sealed class mysqlapisingleton
    {
        ...
        private void setconnectionstring()
        {
            try
            {
                _connectionstringdto = new mysqlconnectionstringdto();
        ...
        public responsedto createdatabasegivennamefromconsole(string new_database_name)
        {
            ...
            try
            {
                _connectionstringdto = getmysqlconnectionstringdto();
                _connectionstringdto.database = "mysql";
        ...
        public bool createdatabase()
        {
            try
            {
                string _default_database_name = utilzsingleton.getInstance(_notificationmessageEventname).getappsettinggivenkey("mysql_database", "inpatient_management_db");
            ...
            try
            {
                ...
                if (!does_doctors_table_exist_in_db)
                {
                    ...
                    string SQL_CREATE_DOCTORS_TABLE = " CREATE TABLE IF NOT EXISTS " + DBContract.doctors_entity_table.TABLE_NAME + " (" +
                          DBContract.doctors_entity_table.ID + " INT NOT NULL AUTO_INCREMENT PRIMARY KEY, " +
            ...
            try
            {
                ...
                using (var con = new MySqlConnection(CONNECTION_STRING))
                {
                    ...
                    using (var cmd = new MySqlCommand(query, con))
                    {
                        ...
                        _responsedto.responsesuccessmessage += "successfully created table [ " + table_name + " ] in [ " + DBContract.mysql + " ].";
        ...
        bool checkiftableexists(string CONNECTION_STRING, string table_name)
        {
            try
            {
                // The query uses parameters for the database and table names, preventing SQL injection.
