...
namespace inpatient_management_system.dal
{
    public sealed class postgresql_api
    {
        ...
        public responsedto save_doctor_in_postgresql(doctor_dto _doctor_dto)
        {
            ...
            if (_exists_in_db)
            {
                ...
                _responsedto.responsesuccessmessage = "doctor with name [ " + _doctor_dto.name + " ] exists in " + DBContract.postgresql + ".";
            ...
            else
            {
                ...
                if (_save_in_db)
                {
                    _responsedto = postgresqlapisingleton.getInstance(_notificationmessageEventname).create_doctor_in_database(_doctor_dto);
        ...
        {
            bool _exists_in_db = postgresqlapisingleton.getInstance(_notificationmessageEventname).check_if_doctor_exists(entity_name);
